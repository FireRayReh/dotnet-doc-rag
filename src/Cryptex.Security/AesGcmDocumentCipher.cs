...
public sealed class AesGcmDocumentCipher : IDocumentCipher
{
    ...
    public AesGcmDocumentCipher(string base64Key, ILogger<AesGcmDocumentCipher>? logger = null)
    {
        if (string.IsNullOrWhiteSpace(base64Key))
            throw new ArgumentException("A base64-encoded 256-bit encryption key is required. Set it via the CRYPTEX_AT_REST_KEY environment variable.", nameof(base64Key));
    ...
    /// <summary>Generates a fresh base64-encoded 256-bit key, useful for provisioning CRYPTEX_AT_REST_KEY.</summary>
