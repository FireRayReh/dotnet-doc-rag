...
public sealed class RagApiController : ControllerBase
{
    ...
    /// id for the uploaded file; generated if omitted), user_id, password (for password-protected
    /// Office/PDF documents), key_id (which configured envelope key decrypts a customer-encrypted
    /// blob; the configured default is used when omitted), and envelope (force envelope decryption
    /// for a blob whose file name doesn't carry a configured encrypted extension).
    /// </summary>
    [HttpPost("/embed")]
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = await _ingestionService.IngestAsync(
                stream, file.FileName, file_id, user_id, password,
                sourceLabel: "librechat-upload",
                envelopeKeyId: key_id,
                forceEnvelope: envelope ?? false,
                cancellationToken: cancellationToken);
        ...
        {
            // Uniform, opaque failure: never tell the caller whether it was a wrong key, bad
            // padding, a failed MAC or a truncated blob - that distinction is a padding oracle.
            // The specific reason is logged at Debug level inside the decryptor, server-side only.
            _logger.LogInformation("Envelope decryption rejected an upload ({FileName}).", file.FileName);
            return UnprocessableEntity(new EmbedResponse { Status = false, FileId = file_id ?? string.Empty, FileName = file.FileName, Error = EnvelopeDecryptionException.UniformMessage });
        }
