...
namespace Cryptex.Api.Contracts;
...
public sealed class BulkIngestRequest
{
    ...
    /// <summary>
    /// Which configured envelope key decrypts customer-encrypted files in this folder. Omit to use
    /// <c>Envelope:DefaultKeyId</c>.
    /// </summary>
    [JsonPropertyName("envelope_key_id")]
    public string? EnvelopeKeyId { get; init; }

    /// <summary>
    /// Optional per-file-name envelope key id overrides, for folders whose files were encrypted
    /// under different keys (key rotation).
    /// </summary>
    [JsonPropertyName("envelope_key_ids_by_file")]
    public Dictionary<string, string>? EnvelopeKeyIdsByFile { get; init; }

    /// <summary>
    /// Treat every file in the folder as a customer-encrypted envelope, even when its name carries
    /// none of the configured encrypted extensions.
    /// </summary>
    [JsonPropertyName("force_envelope")]
    public bool ForceEnvelope { get; init; }
}
